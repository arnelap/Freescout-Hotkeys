...
(function () {
  ...
  function savedRepliesOpen() {
    return !!document.querySelector('.note-btn-group.open .dropdown-saved-replies');
  }

  function savedRepliesToggle() {
    return document.querySelector('button[aria-label="Saved Replies"]');
  }
  ...
  document.addEventListener('keydown', function (e) {
    ...
    if (e.key === '/') {
      if (savedRepliesOpen()) {
        // Second /: close menu, prevent Firefox quick find, then type / in editor
        e.preventDefault();
        savedRepliesToggle()?.click();
        // Re-focus editor and insert a /
        const editor = document.querySelector('.fr-element, .note-editable, [contenteditable="true"]');
        if (editor) {
          editor.focus();
      ...
      if (inEditor()) {
        const toggle = savedRepliesToggle();
